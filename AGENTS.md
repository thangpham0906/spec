...
## Quy tắc đọc code trước khi tạo spec (bắt buộc)

Khi prompt có `Project:` và `Phạm vi biết chắc:` không trống:
- **Bắt buộc đọc code thực tế** trong scope đó trước khi tạo `spec.md`
- Mục đích: biết module đang có gì, tránh duplicate, hiểu convention đang dùng
- Thứ tự đọc:
  1. Code trong `app/code/<Vendor>/<Module>/` thuộc phạm vi đã chốt
  2. `projects/<tên-project>/glossary.md` nếu tồn tại
  3. Spec cũ tại `projects/<tên-project>/features/<tên-feature>/` nếu tồn tại
- Sau khi đọc: ghi rõ trong spec "Đã kiểm tra code hiện tại: <những gì phát hiện>"
- Nếu `Phạm vi biết chắc` để trống: hỏi user trước khi đọc lan man

---

## Quy tắc đặt feature (bắt buộc)

- Nếu prompt có `Project: <tên>` → đặt spec tại `projects/<tên>/features/<feature-name>/`
- Nếu prompt **không có** `Project:` → đặt spec tại `features/<feature-name>/` (core/reusable)
...
- **Không tạo bất kỳ file nào** (spec/plan/tasks/status)
- **Không trigger spec workflow**
- Đọc các tài nguyên liên quan trước khi trả lời theo thứ tự:
  1. Code trong repo (file/module liên quan đến câu hỏi)
  2. `.spec/config/references/` — reference pattern phù hợp
  3. `.spec/examples/` — blueprint nếu có liên quan
  4. `.spec/config/constitution.md` + `.spec/config/magento-patterns.md`
- Trả lời ngắn gọn, có dẫn chứng từ code/reference thực tế trong repo



1. `config/constitution.md` — chuẩn code, quy tắc SOLID, DoD, scope governance
2. `config/magento-patterns.md` — index các pattern + link references
3. `config/glossary.md` — thuật ngữ Magento core
4. `projects/<tên-project>/glossary.md` — thuật ngữ domain riêng (chỉ khi có `Project:`)

## Quy tắc đọc references (bắt buộc)

- `magento-patterns.md` là **index** — mỗi pattern có link tới `config/references/` (path đầy đủ: `.spec/config/references/<category>/<file>.md`).
- Thư mục references nằm tại: `.spec/config/references/` với các sub-folder: `business/`, `core/`, `frontend/`, `infrastructure/`, `inventory/`, `network/`, `ops/`, `security/`.
- **Bước plan:** khi thiết kế `plan.md`, xác định patterns sẽ dùng → đọc references tương ứng → ghi vào section `References sử dụng` trong `plan.md`.
- **Bước implement:** khi implement từng task, đọc lại reference của pattern đó → ghi vào field `Reference đọc trước khi implement` trong task.
- Không được design plan hoặc implement mà không đọc reference — không dựa vào memory.

## Quy tắc đọc examples (bắt buộc)

- `examples/INDEX.md` là index — đọc file này trước, không đọc tất cả blueprint.
- Trigger bắt buộc đọc examples:
  - Tạo module mới từ đầu → đọc `examples/integration/module-skeleton/`
  - Implement feature có dạng tương tự blueprint (REST API, GraphQL, Admin Grid, Message Queue...) → chọn đúng blueprint từ INDEX, đọc trước khi tạo `plan.md`
- Khi tạo `plan.md`, nếu có blueprint phù hợp phải ghi rõ: `Blueprint tham khảo: examples/integration/<tên-file>.md`
- Không được implement module/feature từ đầu mà không kiểm tra examples trước.
...
## Vai trò AI theo từng bước (Role-based behavior)
...
## Phát hiện vấn đề trong quá trình implement (Spec Amendment Flow)

Khi implement, AI có thể phát hiện:
- Thiếu acceptance criteria quan trọng
- Edge case chưa được cover
- Technical constraint ảnh hưởng scope
- Dependency/risk mới phát sinh

### Quy tắc xử lý

**KHÔNG được tự update `spec.md` hoặc `plan.md` mà không có xác nhận.**

Thay vào đó:

1. **Ghi vào `status.md`** section `⚠️ Pending Clarification`:
   ```markdown
   ## ⚠️ Pending Clarification
   
   ### [Task X] Phát hiện vấn đề
   - **Vấn đề:** <mô tả ngắn gọn>
   - **Impact:** <ảnh hưởng đến scope/AC/plan>
   - **Đề xuất:** <1-2 options để xử lý>
   - **Cần xác nhận:** <câu hỏi cụ thể cho human>
   ```

2. **DỪNG task hiện tại**, báo human và chờ xác nhận.

3. **Sau khi human trả lời `OK update spec`:**
   - Cập nhật `spec.md` hoặc `plan.md` với nội dung đã chốt
   - Ghi rõ `[Updated during implementation]` vào dòng thay đổi
   - Xóa section `⚠️ Pending Clarification` khỏi `status.md`
   - Tiếp tục implement

### Lệnh xác nhận

- `OK update spec` — cho phép AI cập nhật spec.md/plan.md với nội dung đã thảo luận
- `Skip` — bỏ qua vấn đề này, tiếp tục implement theo spec cũ
- `Defer` — ghi vào backlog, xử lý sau (không block task hiện tại)
